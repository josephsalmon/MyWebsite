...
**[We need you]{.underline}**: come and help improving it!
...
- **Conformal Prediction** with long tail data [@Ding_Fermanian_Salmon25] : providing prediction sets with statistical guarantees
...
<!-------------------------------------------------------------------------- -->
### Pl@ntNet300K: Long tail visualisation
<hr>
<br>
...
<!------------------------------------------------------------------------ -->
# Prediction & uncertainty quantification {.section-background}
<!------------------------------------------------------------------------ -->
...
{{< iconify mdi:arrow-right-bold >}} **Note**: use as conformal score $s(x,y) = \hat{p}(y|x)$ (= softmax score in deep learning)
...
Conformal prediction sets $t_{\alpha}$ as follows:
$t_{\alpha}$ is the $(1-\alpha)$ quantile of the scores on a calibration set.
...
### Limits of class conditional
<hr>

Ideally you would like to use (label) class-conditional to have stronger guarantees:

In practice, the long-tail is killing you : too many classes with very few labels:
...
<div id="plantnet-viz-2" style="max-width: f; margin: 0 auto;">

  <div id="filter-container-2" style="margin-bottom: 20px;"></div>

  <div style="display: flex; flex-direction: row; align-items: flex-start; gap: 20px;">
  <div id="plotly-container-2" style="flex: 4; height: 400px; width: 48vw; max-width: 48vw; min-width: 200px;"></div>

    <div style="flex: 1; min-width: 280px; display: flex; flex-direction: column;">
      <div style="display: flex; justify-content: flex-end; margin-bottom: 15px;">
        <div class="scale-toggle">
          <span class="scale-label">Y-Scale:</span>
          <button type="button" id="log-scale-btn-2" class="scale-btn"
            onclick="window.setScale2('log'); return false;">Log</button>
          <button type="button" id="linear-scale-btn-2" class="scale-btn active"
            onclick="window.setScale2('linear'); return false;">Linear</button>
        </div>
      </div>

      <div id="image-preview-container-2" style="display: flex; flex-direction: column; justify-content: flex-start; align-items: center; flex: 1;">
        <div style="margin-bottom: 10px;">Species Preview</div>
        <img id="preview-image-2" src="" style="max-width: 100%; height: auto; max-height: calc(100vh - 300px); min-height: 100px; border: 1px solid #ccc; display: none; border-radius: 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); object-fit: contain;" />
        <div id="image-label-2" style="font-weight: bold; font-size: 13px; min-height: 3.5em; margin-top: 10px; text-align: center; width: 100%; line-height: 1.2;"></div>
      </div>
    </div>
  </div>
</div>
...
<script type="module">
  import { initializePlantNetViz } from '../../../../blog/long-tail/plantnet_viz.js';
  
  window.setScale2 = function(scale) {
    document.dispatchEvent(new CustomEvent('scaleChange2', { detail: scale }));
    return false;
  }
  
  initializePlantNetViz({
    vizId: 'plantnet-viz-2',
    filterContainerId: 'filter-container-2',
    plotContainerId: 'plotly-container-2',
    previewImageId: 'preview-image-2',
    imageLabelId: 'image-label-2',
    imagePreviewContainerId: 'image-preview-container-2',
    logScaleBtnId: 'log-scale-btn-2',
    linearScaleBtnId: 'linear-scale-btn-2',
    scaleChangeEvent: 'scaleChange2'
  });
</script>
...
**{{< iconify mdi:bullseye-arrow >}} Goal:**  handle different weight per classes
...
{{< iconify mdi:arrow-right-bold >}} Introduce a new conformal score,  **Prevalence Adjusted Softmax (PAS)** : $s(x,y) = -\omega(y) \frac{\hat{p}(y|x)}{\hat{p}(y)}$
...
To target coverage of at-risk species in Pl@ntNet-300K,  weight by $\gamma \geq 1$ times more at-risk species:
