...
w = np.ones(n_sample)
z = rng.integers(-50, 100, size=(n_sample,)) + 50. * np.log(1 + np.arange(n_sample))
...
def create_signal_from_target(target, y):
    signal = np.zeros_like(y)
    for i, block in enumerate(target):
        signal[block] = np.mean(y[block])
    return signal

# sklearn style
# sol, targets, idx = isotonic_regression(z, w)
sol, _ , idx, targets = isotonic_regression(z.tolist(), w.tolist())
...
def plot_targets(y, targets, x):
    ...
    xaxis_title="X", yaxis_title="Y", template='simple_white', xaxis=dict(range=[-0.5, n+0.5]), yaxis=dict(range=[0, 300]), xaxis2=dict(range=[0, len(targets)]), yaxis2=dict(range=[0, 50000]),
    )
...
def pool_adjacent_violators(y, w):
    ...
    while i < n:
        if r[i] < r[i - 1]:  # Find adjacent violators
            ...
            i += 1
    signal = np.zeros_like(y)
    for _, block in enumerate(target):
        signal[block] = np.mean(np.array(y)[block])
    return signal, target, ws
...
adjusted_y, target, adjusted_w = pool_adjacent_violators(z.tolist(), w)
...
Now, from the values of $x^{\star}_{\underline{b}} = \ldots = x^{\star}_{\overline{b}}$ on the block $B$, one can infer (by recursion) the $\lambda_i$'s for $i\in B$.
...
$$
\begin{align*}
\lambda^{\star}_{i} & = \Big(\sum_{j \in B \cap \llbracket 1, i \rrbracket} w_j \Big)\left( \bar{y}_{B \cap \llbracket 1, i \rrbracket} - \bar{y}_{B}\right) \enspace.
\end{align*}
$$
...
For $i=\underline{b}$ then @eq-stationarity leads to $\lambda^{\star}_{\underline{b}} = w_{\underline{b}} (\bar{y}_{\underline{b}} - x^{\star}_{\underline{b}}) = \Big(\sum_{j \in B \cap \llbracket 1, i \rrbracket} w_j \Big)\left( \bar{y}_{B} - \bar{y}_{B \cap \llbracket 1, i \rrbracket} \right)$.
Now by induction assume the result for $i\in B$, \emph{e.g.,} $\lambda^{\star}_{i}  = \Big(\sum_{j \in B \cap \llbracket 1, i \rrbracket} w_j \Big)\left( \bar{y}_{B \cap \llbracket 1, i \rrbracket} - \bar{y}_{B}\right)$.
Using @eq-stationarity for $i+1 \in B$ yields

$$
\begin{align*}
\lambda^{\star}_{i+1}
& = w_{i+1} (y_{i+1} - x^{\star}_{i+1}) + \lambda^{\star}_i\\
& = w_{i+1} (y_{i+1} - \overline{y}_{B}) + \lambda^{\star}_i\\
...
Hence, the algorithm aims at creating the blocks of constant values in the primal vector. The dual variables can be inferred from the primal solution from the previous lemma.
...
::: {.proof}
To show the convergence we will create a dual variable $\lambda$, show that it is dual feasible, and that at the end of the algorithm it satisfies KKT along with the output of the PAVA algorithm.
...
**Fact 2.** If you update $\lambda$ recursively, starting from $\lambda = \mathbf{0}_{n-1}$ and for if for each updated block $B \in J$ you update $\lambda$ (as in @lem-lambda) by

$$
\begin{align*}
\forall i \in B, \lambda_i & = \Big(\sum_{j \in B \cap \llbracket 1, i \rrbracket} w_j \Big)\left( \bar{y}_{B \cap \llbracket 1, i \rrbracket} - \bar{y}_{B}\right) \enspace,
\end{align*}
$$
then $\lambda$ is dual feasible for all steps of the algorithm

The proof is a direct consequence of Fact 1 and the fact that the weights $w_i$ are non-negative.


**Fact 3.**
$\forall i \in B, \lambda_i=\lambda_{i-1} +w_i(y_i -x_i)$ (where we assume that $x_i = \overline{y}_B$), which means that $\lambda$ and $x$ hence created satsfies.

Again this result is simple to prove, by following the same line in the proof of @lem-lambda.
